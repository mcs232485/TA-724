...
class BBTopo(Topo):
    ...
    def build(self, n=2):
        ...
        bw_list = [args.bw_host, args.bw_net]
        for i in range(n):
            self.addLink(hosts[i], switch, bw=bw_list[i], delay=args.delay, max_queue_size=args.maxq)
...
def start_iperf(net):
    ...
    IP_on_h2 = h2.IP()
    h1 = net.get('h1')
    popen = h1.popen("iperf -c %s -t %s > %s/iperf.out" % (IP_on_h2, args.time, args.dir), shell=True)
...
def start_ping(net):
    ...
    h2 = net.get('h2')
    IP_on_h2 = h2.IP()
    popen = h1.popen("ping -i 0.1 %s > %s/ping.txt" % (IP_on_h2, args.dir), shell=True)
...
def bufferbloat():
    ...
    def fetchWebpage():
        return h2.popen("curl -o /dev/null -s -w %%\{time_total\} %s/http/index.html" % IP_on_h1, shell=True).stdout.readline()
    
    data = []
    timeToFetch = open("%s/timeToFetch" % args.dir, 'w')
    counter = 0

    start_time = time()
    while True:
        # do the measurement (say) 3 times.
        fetchTime = fetchWebpage()
        data.append(float(fetchTime))
        timeToFetch.write(fetchTime)
        timeToFetch.write("\n")
        sleep(5)
        now = time()
