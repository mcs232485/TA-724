...
class BBTopo(Topo):
	...
	# link h1 (remote server) to s0
	#self.addLink(hosts[0], switch, bw=1000)

	#link h2 (home router) to s0
	#self.addLink(hosts[1], switch, bw=10, delay='3ms', max_queue_size=100)
...
def start_iperf(net):
    ...
    #client = h1.popen("iperf -c %s -t 30 > iperf_output.txt" % (h2.IP()), shell=True)
...
def bufferbloat():
	...
	h2 = net.get('h2')
        for i in range(3):
            webpage_time = h2.popen('curl -o /dev/null -s -w %%{time_total} %s/http/index.html' %h1.IP()).communicate()[0]
            fetch_time.append(float(webpage_time))
        sleep(3)
    ...
    #    f.write("Average: %lf\nStandard Deviation: %lf\n" %(avg(fetch_time), stdev(fetch_time)))
