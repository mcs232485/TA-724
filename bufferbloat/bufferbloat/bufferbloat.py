...
class BBTopo(Topo):
    ...
    def build(self, n=2):
        ...
        l1 = self.addLink(hosts[0], switch, bw=args.bw_host, delay=args.delay, max_queue_size=args.maxq)
        l2 = self.addLink(hosts[1], switch, bw=args.bw_net, delay=args.delay, max_queue_size=args.maxq)
...
def start_iperf(net):
    ...
    client = h2.popen("iperf -c %s -t %s > %s/iperfoutput" % (h1.IP(), args.time, args.dir), shell=True)
...
def bufferbloat():
    ...
    def fetch_time():
        h1 = net.get('h1')
        h2 = net.get('h2')
        return float(h2.popen('curl -o /dev/null -s -w %%{time_total} %s/http/index.html' % h1.IP()).communicate()[0])
    ...
    while True:
        ...
        fetches.append(fetch_time())

    print("Average fetch time is " + str(sum(fetches)/len(fetches)))
