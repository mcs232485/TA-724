...
class BBTopo(Topo):
    ...
    def build(self, n=2):
        ...
        #links with appropriate characteristics
        info("link switch to hosts")
        self.addLink(switch, hosts[0], bw=args.bw_host, delay='%sms' % args.delay, max_queue_size=args.maxq)
...
def bufferbloat():
    ...
    print "Mean time = " 
    mean = np.array(rec).astype(np.float)
    print np.mean(mean)
   
    stop_tcpprobe()
    # print("stopped tcpprobe")
    if qmon is not None:
        qmon.terminate()
    # net.stop()
