...
class BBTopo(Topo):
	...
	print(args.maxq)
        self.addLink(h1,s0,bw=1000, delay='3ms', max_queue_size= args.maxq)
        self.addLink(h2,s0,bw=10, delay = '3ms', max_queue_size= args.maxq)
...
# there is a chance that the router buffer may not get filled up.
    ...
    h1=net.get('h1')
    h1.popen('iperf -c %s -t 60 > %s/perf.txt'%(h2.IP(), args.dir), shell = True)
...
def start_fetch(net):
    ...
    start_time = time()
    while True:
       li.append(h2.popen('curl -o /dev/null -s -w %%{time_total} %s/http/index.html' %h1.IP()).communicate()[0])
       li.append(h2.popen('curl -o /dev/null -s -w %%{time_total} %s/http/index.html' %h1.IP()).communicate()[0])
       li.append(h2.popen('curl -o /dev/null -s -w %%{time_total} %s/http/index.html' %h1.IP()).communicate()[0])
       sleep(5)
       now = time()
...
def bufferbloat():
    ...
    p1= Process(target=start_ping(net))
    #p1.start()
    ...
    p2= Process(target = start_iperf(net))
    start_webserver(net)
    start_fetch(net)
    p1.start()
    p2.start()
   ...
   # CLI(net)
    ...
    #h1=net.get('h1')
    #while True:
     #   li.append(h1.cmd('curl -o /dev/null -s -w %{time_total} h2'))
#	li.append(h1.cmd('curl -o /dev/null -s -w %{time_total} h2'))
#	li.append(h1.cmd('curl -o /dev/null -s -w %{time_total} h2'))
 ...
 #       sleep(1)
  #      now = time()
   #     delta = now - start_time
    ...
    for i in li:
#	print(i)
	access.append(float(i))
    print(sum(access)/len(access))
    print(n.std(access))
#    print(n.mean(access))
