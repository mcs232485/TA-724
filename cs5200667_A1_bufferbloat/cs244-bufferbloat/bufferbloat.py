...
class BBTopo(Topo):
	...
	alpha = 0
        for h in hosts:
            if alpha==0:
	        self.addLink(h, switch, bw=args.bw_host, delay=args.delay,
		                             max_queue_size=args.maxq)
		alpha+=1
	    else:
		self.addLink(h, switch, bw=args.bw_net, delay=args.delay,
		                             max_queue_size=args.maxq)
...
def start_iperf(net):
    ...
    client_cmd = "iperf -c {} -t {}> {}/iperf.out".format(h2.IP(),args.time,args.dir)
    h1.cmd(client_cmd, shell = True)
    h1.popen("iperf -c {} -t {} > {}/iperf.out".format(h2.IP(), args.time, args.dir), shell=True)
...
def start_ping(net):
    ...
    h1 = net.get('h1')
    h2 = net.get('h2')
    ping_cmd = "ping -i 0.1 {} > {}/ping.txt &".format(h2.IP(), args.dir)
    ping_train = h1.popen(ping_cmd, shell=True)
...
def bufferbloat():
    ...
    while True:
   	...
   	h2 = net.get('h2') 
    	IP_addr = h1.IP()
        for i in range(3):
            fetch_times.append(float(h2.popen('curl -o /dev/null -s -w %%{time_total} %s/http/index.html' %
                    IP_addr).communicate()[0]))
        sleep(5)
    ...
    avg=0.0
    std=0.0
    print("LEN of fetch_times: ",len(fetch_times))
    for elems in fetch_times:
        avg+=elems
        std+=(elems)**2
    avg = avg/(len(fetch_times))
    std = std/(len(fetch_times))
    std-= avg**2
    print("Avg is: ", avg)
    print("STD is: ", std)
