#!/usr/bin/python
"IIT Delhi 2023 COL724 Assignment 1: Bufferbloat"
...
class BBTopo(Topo):
    ...
    def build(self, n=2):
        ...
        self.addLink('h2',switch, bw=args.bw_net, delay="%sms" % args.delay, max_queue_size=args.maxq)
...
def bufferbloat():
    ...
    while True:
        ...
        results = measure_RTT(net, times=3)
        download_RTT_time.extend(results)
        sleep(5)
        now = time()
    ...
    # README and explain.
    average = avg(download_RTT_time)
    standard_deviation = stdev(download_RTT_time)
    print("Average download time is %s" % average)
    print("Standard Deviation is %s" % standard_deviation)
    stop_tcpprobe()
    if qmon is not None:
        qmon.terminate()
    net.stop()
...
def measure_RTT(net, times = 3):
    h1, h2 = net.get('h1', 'h2')
    download_RTT_time = [float(h2.popen('curl -o /dev/null -s -w %%{time_total} %s/http/index.html' % h1.IP()).communicate()[0]) for i in range(times)]
    return download_RTT_time
