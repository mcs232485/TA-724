...
class BBTopo(Topo):
    ...
    def build(self, n=2):
        ...
        # TODO: 
	self.addLink(hosts[0],switch ,bw=1000,delay='6ms',max_queue_size=100)
	self.addLink(hosts[1],switch,bw=10,delay='6ms',max_queue_size=100)	
...
def start_iperf(net):
    ...
    client =  h1.popen("iperf -c %s -t 600" % h2.IP())
...
def start_ping(net):
    # Get references to the hosts h1 and h2
    h1 = net.get('h1')
    h2 = net.get('h2')

    # TODO: Start a ping train from h1 to h2 (or h2 to h1).
    # Measure RTTs every 0.1 second and save to a file.
    # Use redirection to save the ping output to a file.
    # For example, ping ... > /path/to/ping.txt
    # Note: Use shell syntax since you are redirecting output.
    
    # Construct the ping command with the desired options
    ping_cmd = "ping -i 0.1 %s >> %s/ping.txt" % (h2.IP(), args.dir)  # Append to the file

    # Start the ping train using popen with shell=True
    popen = h1.popen(ping_cmd, shell=True)

# Call the function with your Mininet network object
